...
int buscarEnRecurso(char *palabra,char *recursoEditado){
...
//	char nuevoNombre[80];
	...
	separar(opcion, inputBuffer, '&');
	//para el caso de que sea path
	opcion[strlen(opcion)]=';';
	reemplazarCaracteres(opcion,"%2F",'/');
	reemplazarCaracteres(opcion,"%2E",'.');
	int pos =buscarPunto(opcion);
	borrarNombre(opcion,pos);
	
	DIR *dir;
	 if((dir = opendir(opcion) == NULL)){
        
        printf("<scan style='color: red;'>¡Ups! No se pudo actualizar</scan><br>");
	printf("\n<scan style='color: red;'>No existe el directorio </scan><br>\n");
	...
	//edito el recursoeditado
	char *pal="path";
        int read=buscarEnRecurso(pal,recursoEditado);//busca en que pos esta el path
	convertir(recursoEditado,read+strlen(pal)+2);//convierto ; el anterior path
	removerCaracteres(recursoEditado,";");
       
	insertarSubcadena(recursoEditado,opcion,read+strlen(pal)+2);//actualizo el path
	...
	insertarSubcadena(sAux,recursoEditado,p+tamPalabra);//inserto el nuevo path
	guardoCambios(sAux);
	removerCaracteres(sAux,";");//el anteriorREcurso es ;
        int nuevoPos=strlen(sAux);
	insertarSubcadena(sAux,";",nuevoPos-1);//para el eliminar linea demas
